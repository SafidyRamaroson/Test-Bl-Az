...
export function Header() {
    ...
    return (
        <header
            className={cn(
                "fixed top-0 left-0 w-full z-50 backdrop-blur-xs transition-all duration-300 ease-in-out",
                scrolled && "backdrop-blur-md bg-black/20"
            )}
        >
            <div
                className={cn(
                    "w-11/12 lg:w-3/4 mx-auto border-b border-white flex flex-row justify-between items-center py-3 transition-all duration-300 ease-in-out",
                    isMobile && "w-4/5"
                )}
            >
                {/* Logo */}
